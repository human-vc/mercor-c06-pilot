...
## Sizing from pilot variance (Sept 17; planning inputs, provisional)

Two quantities need separate sizing. The primary claim concerns the overall score shift over all attempts, where an attempt that never contacts the client contributes no author-minus-simulator difference by construction; the shift among contacted states is the secondary quantity. Initial runs and continuation repeats are different things: four initial runs per task generate contact states; continuation repeats are separate runs from each saved state under each reply source.

Pilot inputs: pooled within-state SD of per-task Mean Score across repeated continuations from the same reply, 0.228 (34 reply groups, 36 degrees of freedom, Gemini 3.8 Flash and Qwen 3.8 Flash); between-task SD of base scores 0.42; SD of the paired complete-minus-adapted difference 0.37 over eight tasks. The between-state SD of the true author-minus-simulator shift is unknown and is taken as 0.10 below; the shift among contacted states is taken as 0.10. The 0.228 figure is a provisional planning input: the pre-registered grading rules change how multi-scenario answers score and will change the measured variance.

Planned scale: 72 held-out tasks times four initial runs is 288 attempts per model. Contacted states are 288 times the contact rate, which the pilot puts anywhere from 0 to 0.5 depending on model and prompt (Flash 0 of 8 and 2 of 8; Qwen 1 of 8 and 3 of 8; Fable 4 of 8); at 0.4 that is about 115 contacted states before exclusions. The expert-reply budget is a ceiling, not a guarantee that enough qualifying states exist.

Half-width of a 95 percent interval on one model's overall shift, in points, at 288 attempts, with r continuation repeats per reply source per state (variance per attempt = p·[s_b² + 2·0.228²/r + 0.10²·(1 − p)]):

| Contact rate p (contacted states) | r = 1 | r = 2 | r = 4 |
|---|---:|---:|---:|
| 0.10 (29) | 1.3 | 1.0 | 0.8 |
| 0.25 (72) | 2.0 | 1.5 | 1.2 |
| 0.40 (115) | 2.5 | 1.9 | 1.5 |
| 0.50 (144) | 2.8 | 2.1 | 1.7 |

Among contacted states alone, a two-point half-width on the mean shift needs about 250 to 350 states per model at four repeats (500 to 600 at two) when the between-state SD is 0.10 or less, and 634 at four repeats when it is 0.20.

Reading. An interval half-width of two points does not deliver an equivalence conclusion. For an 80 percent probability that a 95 percent interval falls entirely within ±2 points, with a true shift of zero and a normal approximation, the standard error must be at or below 2/(1.96 + 1.282) = 0.62 points, a half-width near 1.2; both boundaries must be satisfied, and at a standard error of 0.71 points the probability is about 61 percent. This is single-contrast planning; meeting all three interval conditions at once requires a joint power analysis. The difference between two models' shifts, if the estimates are independent, has an interval about √2 wider and needs about twice the attempts for the same precision; shared-task covariance can reduce that. Attempts cluster within tasks and worlds, which widens intervals relative to these independent-attempt figures. Consequences already in the design: the development phase measures contact rates and paired variability under the final grading rules before the confirmatory sample is fixed; the allocation rule spends the expert budget on repeats or on states according to which variance dominates; the confirmatory analysis is frozen before held-out outcomes are examined; and if the achieved interval does not support equivalence, the report states the interval.

## Grading rules to pre-register (from pilot findings)

1. Primary-answer scoring: for a task that requires a single value or recommendation, a criterion is met only if the required value is the answer's stated primary result; values that appear only in a sensitivity, alternative or "if" clause do not count. Tasks that explicitly request a scenario analysis get their own rule, written per task by the author. The pilot's judges credited such clauses inconsistently (3M branches, both judges).
2. Unstated rubric assumptions: before confirmatory runs, the task author lists the assumptions the reference answer relies on that the instruction does not state (for 3M, gross rather than fee-net proceeds); each is added to the instruction, added to the approved disclosures, or accepted as a judgment call whose alternatives are graded as correct. Acceptable answers are settled before any confirmatory output exists.
3. Two judges grade every criterion, and any disagreement is adjudicated by a person blind to reply source and model identity before the score enters the analysis. Agreement between judges is reported but is not treated as validation, since two judges can share a mistake.
